...
struct symbol_table *symbol_table_create(const char *filename)
{
	...
	printf("[ELF READER] ERROR DNYSYM ");
        goto out_unmap;
    }

    table = malloc(sizeof(struct symbol_table));
    if(!table) {
	printf("[ELF READER] ERROR TABLE ");
        goto out_unmap;
    }
    ...
    if(!table->symbols) {
        free(table);
        table = NULL;
	printf("[ELF READER] TABLE SYMBOLS ");
        goto out_unmap;
    }
